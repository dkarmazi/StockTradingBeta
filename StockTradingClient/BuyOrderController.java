...
public class BuyOrderController implements Initializable {
	...
	@FXML
	private ListView<KeyValuePair> PendingOrdersListView = new ListView<>();
	@FXML
	private ComboBox<KeyValuePair> CustomerComboBox = new ComboBox<>();
	@FXML
	private ChoiceBox<KeyValuePair> StockChoiceBox = new ChoiceBox<>();
	@FXML
	private Label SessionStart;
	@FXML
	private Label SessionParams;
	@FXML
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		String sessParams = "Price increase: " + uBound + "% | Price decrease "
				+ lBound + "%";
	...
	public void onAddButtonClick() {
		...
		if (!isNumeric(rCustomerId) || !isNumeric(rStockId)
				|| !isNumeric(rAmount)) {
		...
		try {
			ServerAuthRes sar = si.placeBuyingOrder(o, lBound, uBound,
					clientSessionID);
	...
	public void PopulatePendingOrders(int typeId) {
		...
		try {
			ServerAuthRes sar = si.selectOrdersByFirmByType(brokerageFirmId,
					typeId, clientSessionID);

			if (sar.isHasAccess()) {
				ArrayList<Order> allPendingOrders = (ArrayList<Order>) sar
						.getObject();
				for (Order order : allPendingOrders) {
					PendingOrdersListView.getItems().add(
							new KeyValuePair(Integer.toString(order
									.getOrderId()), order.getDisplaySummary()));
				}
			} else {
				Message.setText("Authorization failed");
			}
	...
	public void PopulateCustomers() {
		...
		try {
			ServerAuthRes sar = si.selectCustomersByFirm(brokerageFirmId,
					clientSessionID);

			if (sar.isHasAccess()) {
				ArrayList<CustomerInfo> allCustomers = (ArrayList<CustomerInfo>) sar
						.getObject();

				for (CustomerInfo c : allCustomers) {
					CustomerComboBox.getItems().add(
							new KeyValuePair(Integer.toString(c.getId()), c
	...
	public void PopulateStocks(int customerId) {
		...
		try {
			ServerAuthRes sar = si
					.selectBuyerCustomerStocksLimited(clientSessionID);

			if (sar.isHasAccess()) {
				ArrayList<Stock> allStocks = (ArrayList<Stock>) sar.getObject();

				for (Stock c : allStocks) {
					StockChoiceBox.getItems().add(
							new KeyValuePair(Integer.toString(c.getId()), c
									.getName()));
				}
			} else {
	...
	private void handleDeleteButtonAction(ActionEvent event) throws Exception {
		...
		String rOrderId = PendingOrdersListView.getSelectionModel()
		...
		try {
			ServerAuthRes sar4 = si.deleteOrder(orderId, clientSessionID);

			if (sar4.isHasAccess()) {
				Validator v = (Validator) sar4.getObject();
	...
	public void ShowDetails() {
		...
		try {
			...
			if (sar.isHasAccess()) {
				...
				StockChoiceBox.getItems().clear();
				StockChoiceBox.getItems().add(
						new KeyValuePair(Integer.toString(o.getStockId()), o
								.getDisplayStockName()));
	...
	private String getChoiceBoxSelection(ChoiceBox<KeyValuePair> cbx) {
		...
		if (cbx == null || cbx.getValue() == null
				|| cbx.getValue().getKey() == null) {
			return out;
		} else {
